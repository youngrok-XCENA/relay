...
generate_relay_caller() {
  local agent="$1" desc="$2" style="$3" exts="$4" build="$5" test_cmd="$6" base="$7"
  local Agent="${agent^}"
  local trigger_cmd="/${agent}-issue"
  local fix_cmd="/${agent}-fix"
  local review_cmd="/${agent}-review"
  local auto_kw="${agent}-auto"
  local prefix="${agent}/fix-issue-"
  local bot="${agent}-ci[bot]"
  local bot_email="${agent}-ci[bot]@users.noreply.github.com"
  local runner_label="${agent}-ci"
  local sandbox_review="read-only"
  local sandbox_fix="read-only"
  local sandbox_auto="read-only"
  if [ "$agent" = "codex" ]; then
    sandbox_fix="full-auto"
    sandbox_auto="full-auto"
  fi
...
jobs:
  review:
    if: >
      startsWith(github.event.comment.body, '${review_cmd}') &&
    ...
    with:
      ...
      trigger_command: "${review_cmd}"
      title_keyword: "${agent}"
      branch_prefix: "${prefix}"
      bot_user: "${bot}"
      bot_email: "${bot_email}"
      runner: '["self-hosted", "${runner_label}"]'
      codex_sandbox: "${sandbox_review}"
      max_review_turns: 10
      timeout_minutes: 15
    secrets:
  ...
  fix:
    if: >
      ...
      startsWith(github.event.comment.body, '${fix_cmd}')
    uses: youngrok-XCENA/relay/.github/workflows/relay.yml@main
    with:
      agent: "${agent}"
      mode: fix
      ...
      (github.event_name == 'issues' &&
       (github.event.action == 'closed' ||
        ((github.event.action == 'opened' || github.event.action == 'reopened') &&
         ...
         !contains(github.event.issue.title, '${auto_kw}')))) ||
      (github.event_name == 'issue_comment' &&
       !github.event.issue.pull_request &&
       startsWith(github.event.comment.body, '${trigger_cmd}') &&
...
render_workflow_content() {
  ...
  case "$workflow" in
    claude|codex) generate_relay_caller "$workflow" "$desc" "$style" "$exts" "$build" "$test_cmd" "$base" ;;
    *)      return 1 ;;
  esac
}
